...
The system SHALL provide a `savePrompt(type, suggestedTitle, suggestedTags, content)` function that interactively asks the user: "💾 Save as {type}? [Y/n]". If
yes, it SHALL prompt for title (pre-filled with suggestion), tags (pre-filled with suggestions), and difficulty (for snippets).
...
After a save prompt (or independently for commands), the system SHALL ask "📋 Copy to clipboard? [Y/n]". If yes, it SHALL copy the content using
`copyToClipboard()`.
