...
The system SHALL provide predefined templates for 4 project types: `cli` (CLI app), `api` (REST API), `script` (single-file utility), `library` (publishable library). Each template defines a base file list, directory structure, and default config.
...
The system SHALL infer the project type from the user's description. Keywords like "CLI", "command", "tool" suggest cli; "API", "server", "REST" suggest api; "script", "utility", "convert" suggest script; "library", "package", "module" suggest library.
