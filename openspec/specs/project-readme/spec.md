...
The system SHALL include a `README.md` at the project root with: project title with ASCII art, description, installation instructions (JSR + deno install + manual), all 7 skills with brief descriptions and usage examples, configuration reference (config.yaml fields), pi integration guide (`coach install-pi`), and license section.
...
The README SHALL document 3 installation methods: (1) `deno install -g jsr:@ssal/dev-coach`, (2) `deno install -g` from source, (3) `deno task install` for development.
