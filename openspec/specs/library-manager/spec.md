...
The system SHALL provide a `saveItem(type, content, metadata)` function that writes a markdown file with YAML frontmatter to the appropriate library subdirectory. For snippets, the file SHALL be placed in `{library}/snippets/{lang}/{slug}.md`. For TLDRs, in `{library}/tldr/{slug}.md`. For projects, in `{library}/projects/{slug}/README.md`.
...
The system SHALL generate kebab-case slugs from titles. If a file with that slug already exists, it SHALL append `-2`, `-3`, etc. until a unique filename is found.
