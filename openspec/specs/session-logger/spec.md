...
The system SHALL provide a `logSession(params)` function that inserts a row into the `sessions` table with: ts (auto-set to current ISO timestamp), mode, lang, tags (JSON array), query, duration_s, and saved_as (optional file path).
