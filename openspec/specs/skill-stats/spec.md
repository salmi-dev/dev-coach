...
`coach stats` SHALL render an ASCII box dashboard showing: total session count (with month-over-month delta), mode breakdown, language bar charts (top 5, block
characters with percentages), library item counts (snippets, tldrs, projects), current streak (consecutive days), most active day of week, and newest topic this
month.
