...
The system SHALL resolve the config directory using `$XDG_CONFIG_HOME/dev-coach/`. If `$XDG_CONFIG_HOME` is not set, it SHALL default to `~/.config/dev-coach/` on macOS/Linux and `%APPDATA%/dev-coach/` on Windows.
...
The system SHALL resolve the data directory using `$XDG_DATA_HOME/dev-coach/`. If `$XDG_DATA_HOME` is not set, it SHALL default to `~/.local/share/dev-coach/` on macOS/Linux and `%LOCALAPPDATA%/dev-coach/` on Windows.
...
The system SHALL resolve the library path from config (`library_path` field). If not configured, it SHALL default to `~/dev-coach/`. The path SHALL support `~` expansion.
