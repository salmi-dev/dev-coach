...
The system SHALL provide a `parseFrontmatter(content)` function that extracts YAML between `---` fences at the start of a markdown file and returns
`{ metadata, body }`.
...
The system SHALL define typed interfaces: `SnippetFrontmatter` (title, tags, created, source, difficulty?, lang), `TldrFrontmatter` (title, tags, created,
source), `ProjectFrontmatter` (title, tags, created, source, lang). All interfaces SHALL include `title: string`, `tags: string[]`, `created: string` (ISO
date), `source: string`.
