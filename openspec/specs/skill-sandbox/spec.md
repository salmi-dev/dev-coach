...
The `coach:sandbox` skill SHALL accept a topic string and produce multiple numbered approaches exploring that topic. Each approach SHALL include a title, code example, and explanation of trade-offs.
...
After the summary, the sandbox skill SHALL use the `ApproachCollector.selectAndSave()` to let the user choose which approaches to save as individual snippets in `snippets/{lang}/`.
