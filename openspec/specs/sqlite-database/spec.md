...
The system SHALL store the SQLite database at `{data_dir}/coach.db`. The system SHALL provide a `getDb()` function that returns a connected database instance, creating the file if it does not exist.
...
The system SHALL track applied migrations in a `_migrations` table with columns `version` (integer) and `applied_at` (text). On startup, the system SHALL execute any migrations with version greater than the last applied version. Migrations SHALL be forward-only (no rollback).
...
The system SHALL create a `sessions` table with columns: `id` (integer primary key), `ts` (text, ISO 8601, not null), `mode` (text, not null), `lang` (text), `tags` (text, JSON array), `query` (text), `duration_s` (integer), `saved_as` (text).
...
The system SHALL create an `items` table with columns: `id` (integer primary key), `type` (text, not null), `title` (text, not null), `path` (text, not null), `lang` (text), `tags` (text, JSON array), `created` (text, not null), `updated` (text), `source_session` (integer, foreign key to sessions.id).
