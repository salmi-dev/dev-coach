...
macOS, `xclip -selection clipboard` or `xsel --clipboard` on X11 Linux, `wl-copy` on Wayland Linux, and `clip` on Windows.
...
`isInteractive()` returning whether stdin is a TTY.
