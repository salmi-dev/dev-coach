...
The system SHALL provide a `coach-save` pi custom tool that agents can call to save a snippet/tldr/project during an interaction. Parameters: `type` (snippet|tldr|project), `title` (string), `content` (string), `lang` (optional string), `tags` (string array). It SHALL call `saveItem()` from the storage layer.
...
The system SHALL provide a `coach-search` pi custom tool that agents can call to search the user's library. Parameters: `query` (optional string), `type` (optional), `lang` (optional), `tags` (optional string array). It SHALL call `search()` from the storage layer and return results.
