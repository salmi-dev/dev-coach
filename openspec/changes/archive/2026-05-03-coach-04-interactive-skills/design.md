...
Changes 01–03 established: Deno CLI, config/DB/XDG, storage layer (CRUD, search, session logging, frontmatter, dashboard, save prompts), and three 1→1 skills (ask, explain, compare) with the base `Skill` interface, `SessionContext`, `SkillResult`, and `runSkill()` runner.

This change adds two multi-response skills that produce multiple outputs per session. The sandbox explores a topic with n approaches, then lets the user batch-select which to save. The review skill reads code from file/stdin/paste and produces structured feedback across 6 categories.
...
**Choice**: An `ApproachCollector` class that accumulates numbered approaches, each with title, content, lang, and tags. At end, provides `selectAndSave()` for batch selection.
...
**Choice**: For file input, detect from extension (`.rs` → rust, `.ts` → typescript, etc.). For stdin/paste, use simple keyword heuristics (fn/let/impl → rust, def/import → python, etc.).
...
**Choice**: `coach-save` and `coach-search` are TypeScript functions that wrap `saveItem()` and `search()` from the storage layer, formatted as pi tool definitions.
