...
The system SHALL accept a `SearchFilters` object with optional fields: `query` (FTS text), `type` (snippet|tldr|project), `lang` (string), `tags` (string array
— match any), `limit` (number).
